...
class FilePersistenceProvider extends AbstractPersistenceProvider {
	...
	function initialize($params) {
		if (is_array ( $params )) {
			if (! array_key_exists ( self::DIR, $params )) {
				throw new \WURFL\Exception ( "Specify a valid Persistence dir in the configuration file" );
			}
			
			// Check if the directory exist and it is also write access
			if (! is_writable ( $params [self::DIR] )) {
				throw new \WURFL\Exception ( "The diricetory specified <" . $params [self::DIR] . "> for the persistence provider does not exist or it is not writable\n" );
			}
	...
	public function save($objectId, $object) {
		...
		\WURFL\FileUtils::write ( $path, $object );
	}
	...
	public function load($objectId) {
		$path = $this->keyPath ( $objectId );
		return \WURFL\FileUtils::read ( $path );
	}

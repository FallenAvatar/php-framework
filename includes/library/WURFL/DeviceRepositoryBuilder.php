...
class DeviceRepositoryBuilder {
	...
	public function build($wurflFile, $wurflPatches = array(), $capabilitiesToUse = array()) {
		if (!$this->isRepositoryBuilt()) {
			...
			if (flock($fp, LOCK_EX | LOCK_NB) && !$this->isRepositoryBuilt()) {
				$infoIterator = new \WURFL\Xml\VersionIterator($wurflFile);
				$deviceIterator = new \WURFL\Xml\DeviceIterator($wurflFile, $capabilitiesToUse);
		...
		return new \WURFL\CustomDeviceRepository($this->persistenceProvider, $deviceClassificationNames);
	}
	...
	private function toPatchIterators($wurflPatches, $capabilitiesToUse) {
		...
		if (is_array($wurflPatches)) {
			foreach ($wurflPatches as $wurflPatch) {
				$patchIterators [] = new \WURFL\Xml\DeviceIterator($wurflPatch, $capabilitiesToUse);
			}
		}
	...
	private function persistWurflInfo($wurflInfoIterator) {
		foreach ($wurflInfoIterator as $info) {
			$this->persistenceProvider->save(\WURFL\Xml\Info::PERSISTENCE_KEY, $info);
		}
	}

...
{
abstract class ActiveRecord extends \System\Object
{
	protected $tbl;
	protected $cols;
	protected $prikeycol;
	...
	protected function __construct($options=null)
	{
		...
		if( !isset($options) || !is_array($options) || count($options) == 0 )
			throw new \Exception('System\Data\ActiveRecord must be initialized with an options array containing { "table","columns","primaryidname" }.');
		
		if( !isset($options['table']) )
			throw new \Exception('System\Data\ActiveRecord must be initialized with an options array containing { "table","columns","primaryidname" }.1');
		
		if( !isset($options['columns']) )
			throw new \Exception('System\Data\ActiveRecord must be initialized with an options array containing { "table","columns","primaryidname" }.2');
		
		if( !isset($options['primaryidname']) )
			throw new \Exception('System\Data\ActiveRecord must be initialized with an options array containing { "table","columns","primaryidname" }.3');
		...
		if( isset($options['id']) )
		{
			...
			if( (!is_array($id) || count($id) == 1) && (!is_array($this->prikeycol) || count($this->prikeycol) == 1) )
				...
				throw new \Exception('Invalid id/data passed to System\\Data\\Table');
		}
	}
	...
	public function Save()
	{
		...
		if($this->loaded)
		{
			...
			$update = new \System\Data\Statement\Update($this->tbl,$where);
		...
		{
			$insert = new \System\Data\Statement\Insert($this->tbl);

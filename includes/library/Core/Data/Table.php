...
class Table extends \Core\Obj {
	...
	public static function CreateFromAR(string $arClass): bool {
		$refClass = null;

		try {
			$refClass = new \ReflectionClass($arClass);
		} catch (\Throwable $t) {
			static::RecordError($t);
			return false;
		}
	}

	private static function RecordError(?\Throwable $t) {
		static::$lastError = $t;
	}
	...
	public function _getDb(): \Core\Data\Database {
		return $this->db;
	}

...
abstract class ActiveRecord extends \Core\Obj implements \JsonSerializable {
	...
	public static function FindAllPaged(int $npp, int $page): array {
		$db = static::GetDb();
		$tbl_name = static::$table_name;
	...
	public static function FindAllBy(array $params, string $type = 'AND', ?array $order_by = null): array {
		$db = static::GetDb();
		$tbl_name = static::$table_name;
	...
	protected static function GetDb(): \Core\Data\Database {
		return \Core\Data\Database::Get(isset(static::$db_name) ? static::$db_name : null);
	}
	protected static function GetTable(): \Core\Data\Table {
		$db = static::GetDb();
		return \Core\Data\Table::Get($db, static::$table_name);
	}
	
	protected static function ParseType(string $name, $type): array {
		$chars = [
			'allow_null' => '?',
			'unsigned' => '+',
			'auto_increment' => '@',
	...
	private static function loadColInfo(): array {
		$cols = [];
		$keys = [];
	...
	public function _getCols(): array {
		return $this->cols;
	}
